...
> JWT 是無狀態的，適合前後端分離架構，不需要伺服器端存 Session，
> 未來如果要水平擴展（多台伺服器）也不用處理 Session 共享問題。
> 醫院環境如果有更嚴格的需求，可以改用 refresh token 機制延長有效期。

**為什麼要做稽核紀錄？**
> 醫院的資料是受《個資法》保護的，任何對個人資料的操作都必須留存紀錄，
> 以便後續稽核或糾紛調查。這不是加分項，而是基本要求。

**如果資料量很大怎麼辦？**
> 目前 attendance_requests 已在 user_id、status、start_date 建立索引，
> 常見的查詢條件都走索引。如果資料量達到百萬級，可以考慮分表或加 Redis 快取待審核計數。
